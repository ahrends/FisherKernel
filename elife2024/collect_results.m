...
for Fn = 1:3 % main kernels
    ...
    for Kn = 1:2
        ...
        for varN = 1:nvars
            for iterN = 1:niter
                load([outputdir '/Results_only_cov' num2str(only_cov) '_' type '_' shape '_varN' num2str(varN) 'iterN' num2str(iterN) '.mat']); % results
                for k = 1:nfolds
                    i = i+1;
...
writetable(resultsT, [outputdir '/MAINresultsT.csv'])
        ...
        for varN = 1:nvars
            for iterN = 1:niter
                if FSn == 1
                    ...
                    load([outputdir '/Results_' featureset '_only_cov' num2str(only_cov) '_' type '_' shape '_varN' num2str(varN) 'iterN' num2str(iterN) '.mat']); % results
                end
                for k = 1:nfolds
                    i = i+1;
...
writetable(featsets_resultsT, [outputdir '/FEATSETSresultsT.csv'])
...
for Fn = 1:nmethods
    ...
    for Kn = 1:nkernels
        ...
        for CVn = 1:nschemes
            CV = all_schemes{CVn};
            for varN = 1:nvars
                load([outputdir '/Results_' CV '_only_cov' num2str(only_cov) '_' type '_' shape '_varN' num2str(varN) 'iterN' num2str(iterN) '.mat']); % results              
                for k = 1:nfolds
                    i = i+1;
