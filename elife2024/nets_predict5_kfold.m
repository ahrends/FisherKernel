function [stats,predictedY,predictedY0,predictedYD,predictedYD0,beta,YD] ...
    = nets_predict5_kfold(Yin,Xin,family,parameters,varargin)
...
% adapted to do fold-wise model evaluation %CA
%
...
leaveout_foldN = parameters.leaveout_foldN; % added this to do fold-wise model evaluation %CA
...
if riemann
    ...
    for j=1:size(Xin,3)
        ev = eig(Xin(:,:,j));
        if any(ev<0)
            warning(sprintf('The matrix for subject %d is not positive definite',j))
            Xin(:,:,j) = nearestSPD(squeeze(Xin(:,:,j)));
        end
    end
...
for perm=1:Nperm
    ...
    if isempty(CVfolds)
        if CVscheme(1)==1
            ...
            folds = cvfolds_FK(Yin,family,CVscheme(1),allcs);
        end
    else
        folds = CVfolds;
    end
    ...
    ifold = leaveout_foldN; % adjusted to do fold-wise model evaluation %CA
    %for ifold = 1:length(folds) 
        ...
        % create the inner CV structure - stratified for family=multinomial
        Qfolds = cvfolds_FK(Y,family,CVscheme(2),Qallcs); % use new cvfolds function to properly randomise while keeping family structure intact
...
% adjusted output for fold-wise model evaluation %CA
%     YinORIG = Yin(J);
%     predictedYp = predictedYp(J);
%     predictedYp0 = predictedYp0(J);
%     predictedYpD = predictedYpD(J);
%     predictedYpD0 = predictedYpD0(J);
    ...
    if perm==1
        ...
        if strcmpi(family,'gaussian')
            errs = abs(predictedY(J)-YinORIG(J));
            mae = max(errs);
            stats.nmae = mae/range(YinORIG(:));
            stats.dev = sum((YinORIG(J)-predictedY(J)).^2);
            stats.baseline_dev = sum((YinORIG(J)-predictedY0(J)).^2);
            stats.corr = corr(YinORIG(J),predictedY(J));
            stats.baseline_corr = corr(YinORIG(J),predictedY0(J));
            stats.cod = 1 - stats.dev / stats.baseline_dev;
            if Nperm==1
                [~,pv] = corrcoef(YinORIG(J),predictedY(J)); stats.pval=pv(1,2);
                if corr(YinORIG(J),predictedYp(J))<0, pv = 1; end
            end
            if deconfounding(2)
                stats.dev_deconf = sum((YD(J)-predictedYD(J)).^2);
                stats.baseline_dev_deconf = sum((YD(J)-predictedYD0(J)).^2);
                stats.corr_deconf = corr(YD(J),predictedYD(J));
                stats.baseline_corr_deconf = corr(YD(J),predictedYD0(J));
                stats.cod_deconf = 1 - stats.dev_deconf / stats.baseline_dev_deconf;
                if Nperm==1
                    [~,pvd] = corrcoef(YD(J),predictedYD(J)); stats.pval_deconf=pvd(1,2);
                end
                errs_deconf = abs(predictedYD(J)-YD(J));
                mae_deconf = max(errs_deconf);
                stats.nmae_deconf = mae_deconf/range(YD(:));
            end
